...
public class Main {
	...
	{		
		...
		double maximumStockLengthCost = 0;
		
		for(int i = 0; i < stockLengths.length; i++) 
		{
			//Find Most expensive stock length cost
			if(stockCosts[i] > maximumStockLengthCost) 
			{
				maximumStockLengthCost = stockCosts[i];
			}
		...
		double maximumOrderCost = 0;
		
		for(int i = 0; i < orderLengths.length; i++) 
		{
			//Calculate Maximum Order Cost by using the most expensive length for each ordered length
			maximumOrderCost += maximumStockLengthCost * orderQuantities[i];
		...
		return new MCutProblem(stockLengthsAndCosts, orderedLengthsAndQuantities, maximumOrderCost);
	}
